...
const PostCard = ({ post, isFavorite = false, onToggleFavorite }) => {
  const postId = post._id || post.id;

  const handleToggle = (e) => {
    e.preventDefault();
    e.stopPropagation();
    onToggleFavorite?.(postId, !isFavorite);
  };
  ...
  return (
    <div className="relative bg-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-100 hover:border-gray-200 group">
      ...
      <button
        ...
        className={[
          "absolute top-3 right-3 z-10 rounded-full px-3 py-1.5 flex items-center gap-2 text-sm font-medium transition backdrop-blur-sm",
          isFavorite
